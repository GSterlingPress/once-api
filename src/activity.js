...
function classify(e){
 ...
 if(e.kind==='mcp'&&e.result==='tools/call:once_execute'){
   ...
   if(INTERACTIVE_CLIENT_RE.test(client))return {classification:'CREDIBLE_REAL_USE',reasons:[`interactive MCP client: ${client}`,'core ONCE tool executed']};
   return {classification:'UNKNOWN_MACHINE',reasons:['core ONCE tool executed','no affirmative interactive-client evidence']};
 }
 if(e.kind==='once'){
